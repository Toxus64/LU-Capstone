...
<?php
  ...
  if (isset($_GET['delete'])) {
	  if($_GET['delete'] == true) {
		$id = $_GET['id'];
		$delete = $conn->query("DELETE FROM orders WHERE orderID = '$id'");
	...
	<div class="content">
		...
		<form class="center">
			<fieldset>
				...
				<button type="button" onClick="searchForOrder(document.getElementById('orderID').value, document.getElementById('customerID').value)"  class="navButton">Search</button>
				
				<button type="button" onClick="createNewOrder()" class="navButton">Create new Order</button> 
			</fieldset>
		</form>
	</div>
</body>
